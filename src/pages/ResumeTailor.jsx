  ...
  return (
    <div className="p-4 md:p-8 max-w-max_width mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium mt-2">Optimize your resume bullets to perfectly match the job requirements.</p>
      </div>

      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-xl p-4 mb-8 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="xl:col-span-5 space-y-6">
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
            ...
            <label className="border-2 border-dashed border-slate-200 dark:border-white/10 rounded-xl p-8 flex flex-col items-center justify-center gap-3 bg-slate-50/50 dark:bg-white/[0.02] hover:bg-slate-50 dark:hover:bg-white/[0.05] transition-colors cursor-pointer group">
              <div className="w-12 h-12 rounded-full bg-white dark:bg-white/10 shadow-sm flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
          ...
          <div className="flex gap-4">
            <button onClick={handleReset} disabled={loading} className="flex-1 py-4 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-200 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-white/10 transition-all">
              <span className="material-symbols-outlined text-[20px]">refresh</span> Reset
            </button>
            <button 
              onClick={handleTailor} 
              disabled={loading} 
              className="flex-[2] py-4 bg-primary text-white rounded-xl font-bold flex items-center justify-center gap-3 shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-60"
            >
              {loading ? (
                <>
          ...
          {result ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              {/* Score Transformation Header */}
               <div className="bg-gradient-to-r from-primary to-indigo-600 rounded-3xl p-8 shadow-xl text-white overflow-hidden relative">
              ...
              <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
                <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 mb-6 uppercase tracking-widest flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {result.improvedPoints?.map((point, i) => (
                    <div key={i} className="group p-5 bg-slate-50 dark:bg-white/[0.02] hover:bg-white dark:hover:bg-white/[0.05] border border-slate-100 dark:border-white/5 hover:border-primary/20 rounded-2xl transition-all relative">
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-200 leading-relaxed pr-10">{point}</p>
                      <button 
                        onClick={() => copyToClipboard(point)}
                        className="absolute top-4 right-4 p-2 bg-white dark:bg-white/10 border border-slate-200 dark:border-white/10 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary/5 hover:border-primary/20 text-slate-400 hover:text-primary"
              ...
              <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {result.suggestions?.map((tip, i) => (
                    <div key={i} className="flex gap-3 p-4 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100/50 dark:border-amber-900/20 rounded-2xl">
